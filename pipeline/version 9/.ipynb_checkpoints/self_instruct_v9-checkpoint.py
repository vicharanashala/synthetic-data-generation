...
# Run     : python self_instruct_v9.py
#
# CHANGES vs v8:
#   FIX-25: NoneType crash guard — `resp.choices[0].message.content` can be
#            None when the API returns a response with no content (e.g. after
#            a refusal or an empty delta). Added an explicit None-check before
#            calling .strip() so the exception no longer wastes a retry slot.
#   FIX-26: Tighter per-style grounding constraints for the three highest-flag
#            styles. New explicit FORBIDDEN lists injected into each description:
#              paraphrase_problem_description (was 62% flag rate):
#                + FORBIDDEN: sowing timing, intercropping, location details,
#                  any fact not present as a symptom/diagnosis/top-2-3-actions.
#              paraphrase_preventive (was 53% flag rate):
#                + FORBIDDEN: chemical dosages not stated, new crop varieties,
#                  sowing-date windows, location details not in seed.
#              paraphrase_cause (was 43% flag rate):
#                + FORBIDDEN: pathogen lifecycle details, host range, alternative
#                  crops, any causal mechanism not explicitly described in seed.
#   FIX-27: paraphrase_comparison minimum word floor lowered from 50 → 30 to
#            stop thrashing (26 calls for 4 pairs) when the seed has only limited
#            comparable items and the model correctly gives a concise answer.
#   FIX-28: Output files renamed to paraphrase_v9.* and IDs use si_v9_ prefix.
#
# CHANGES vs v7 (inherited from v8):
#   FIX-21: Judge is now a HARD GATE — flagged generations are discarded and
#            retried (up to MAX_JUDGE_RETRIES times per slot) instead of being
#            soft-flagged and saved. Only judge_grounded=True pairs reach the
#            final output. The log records JUDGE_RETRY lines so you can track
#            how many extra calls were needed.
#   FIX-22: MAX_JUDGE_RETRIES config knob (default 3) controls how many times
#            a slot is retried after a judge flag before it counts as a failure.
#   FIX-23: Output files renamed to paraphrase_v8.* and IDs use si_v8_ prefix.
#   FIX-24: Summary now reports judge retry count alongside skip/dupe counts.
#
...
#   FIX-17: Judge uses enable_thinking=False (fast, cheap).
#   FIX-18: JUDGE_MAX_TOKENS = 400.
#   FIX-19: Output files renamed to paraphrase_v7.* and IDs use si_v7_ prefix.
#   FIX-20: Summary now reports judge flag rate alongside skip/dupe counts.
#
...
OUT_JSON        = "/home/kritika/self_instruct/paraphrase_v9.json"
OUT_CSV         = "/home/kritika/self_instruct/paraphrase_v9.csv"
OUT_LOG         = "/home/kritika/self_instruct/paraphrase_v9_log.txt"
...
STYLE_MIN_WORDS = {
    ...
    "paraphrase_comparison"         : 30,   # FIX-27: was 50, lowered to 30
    "paraphrase_cause"              : 50,
...
# ─── Style Taxonomy ───────────────────────────────────────────────────────────
# FIX-26: Tighter grounding constraints for the three highest-flag styles.
# FIX-27: paraphrase_comparison minimum word floor moved to STYLE_MIN_WORDS (30).
STYLES = [
    ...
    # FIX-26: Added explicit FORBIDDEN list for problem_description (was 62% flag rate).
    # Root cause: model was appending sowing timing, intercropping suggestions, and
    # location-specific advice that were not part of the seed symptom/diagnosis/action set.
    {
    ...
    # FIX-26: Added explicit FORBIDDEN list for preventive (was 53% flag rate).
    # Root cause: model was adding chemical dosages, new crop varieties, sowing-date
    # windows, and location details that were not in the seed's prevention content.
    {
    ...
    # FIX-26: Added explicit FORBIDDEN list for cause (was 43% flag rate).
    # Root cause: model was adding pathogen lifecycle details, host-range facts,
    # and alternative-crop context that were not stated in the seed.
    {
...
def call_model(seed: dict, style: dict, style_call_count: int,
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # FIX-25: Guard against None content before calling .strip().
            # The API can return message.content=None on refusals or empty deltas.
...
def call_judge(seed_answer: str, generated_answer: str) -> dict:
    ...
    try:
        ...
        # FIX-25: Same None-content guard applied to judge responses.
        raw_content = resp.choices[0].message.content
        if raw_content is None:
            return {"grounded": True, "confidence": "low", "issues": [],
                    "judge_error": "null_content"}
...
def make_generated_id(seed: dict, style_name: str, idx: int) -> str:
    ...
    return f"si_v9_{style_name[:6]}_{base}_{idx}"   # FIX-28
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Self-Instruct Paraphrase v9 (English only, CSV input)")
    ...
    print(f"  FIX-25     : None-content guard active (generation + judge)")
    print(f"  FIX-26     : Tightened grounding for problem_description / preventive / cause")
    print(f"  FIX-27     : comparison min floor = {STYLE_MIN_WORDS['paraphrase_comparison']}w (was 50w)")
    print(f"{'='*60}\n")
        ...
        for style in STYLES:
            ...
            while style_done < TARGET_PER_STYLE:
                ...
                while judge_retry_count <= MAX_JUDGE_RETRIES:
                    ...
                    print(f"⚠ FLAGGED ({judge_confidence}) — retrying (judge retry {judge_retry_count}/{MAX_JUDGE_RETRIES})...",
                          end=" ", flush=True)
                    global_idx += 1
                    result, detected_focus = call_model(
                        seed, style, call_count, used_focus_types, consecutive_unknowns
                    ...
                    if result is None:
                        ...
                        log_lines.append(
                            f"SKIP | seed={seed_idx} | style={style['name']} | call={call_count} | (judge-retry)"
                        )
                        break
    ...
    print(f"  Log         : {OUT_LOG}")
    print(f"  Note: All saved pairs are judge_grounded=True. Check JUDGE_RETRY/JUDGE_ABANDON")
    print(f"        lines in the log to see how many extra calls were needed per slot.")
    print(f"{'='*60}\n")
