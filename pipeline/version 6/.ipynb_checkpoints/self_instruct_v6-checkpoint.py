...
# Run     : python self_instruct_v6.py
#
# CHANGES vs v4:
...
#   FIX-10: Per-style minimum AND maximum word counts — comprehensive styles
#            require ≥50 words; naturally-short styles (yes_no, quantity,
#            timing) are capped at 80-100 words to prevent ballooning.
#            Floors lowered to match natural output: yes_no→20w, quantity→20w,
#            timing→20w, problem_description→25w (was 25/30/30/30).
...
#   FIX-15: enable_thinking=True — Qwen3's built-in chain-of-thought scratchpad
#            is now active. The model reasons internally before producing the
#            JSON output, improving grounding on complex chemical dosages.
#            The thinking tokens are NOT included in the output JSON.
...
OUT_JSON        = "/home/kritika/self_instruct/paraphrase_v6.json"
OUT_CSV         = "/home/kritika/self_instruct/paraphrase_v6.csv"
OUT_LOG         = "/home/kritika/self_instruct/paraphrase_v6_log.txt"
...
STYLE_MIN_WORDS = {
    ...
    "paraphrase_problem_description": 25,   # 3-5 sentences is naturally ~25w
    "paraphrase_followup"           : 40,
    "paraphrase_preventive"         : 40,
    "paraphrase_comparison"         : 50,
    "paraphrase_cause"              : 50,
    "paraphrase_quantity_focused"   : 20,   # dosage answers are short by nature
    "paraphrase_timing_focused"     : 20,   # timing answers are short by nature
    "paraphrase_yes_no"             : 20,   # "Yes/No + 2 sentences" ≈ 20w
    "paraphrase_short"              : 15,   # intentionally short
}

# Per-style MAXIMUM word counts — prevent ballooning in naturally short styles
STYLE_MAX_WORDS = {
    "paraphrase_short"              : 60,
...
def is_answer_length_ok(text: str, style_name: str) -> bool:
    """
    Check answer word count against both global bounds and per-style min/max.
    FIX-10: Each comprehensive style has a higher floor than the global 15-word min.
    Per-style max prevents naturally-short styles from ballooning to 300 words.
    """
...
def call_model(seed: dict, style: dict, style_call_count: int,
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # FIX-10: Per-style length check (min AND max)
...
def make_generated_id(seed: dict, style_name: str, idx: int) -> str:
    ...
    return f"si_v6_{style_name[:6]}_{base}_{idx}"
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Self-Instruct Paraphrase v6 (English only, CSV input)")
    ...
    print(f"  Answer length: global [{MIN_ANSWER_WORDS}–{MAX_ANSWER_WORDS}w], per-style overrides active")
