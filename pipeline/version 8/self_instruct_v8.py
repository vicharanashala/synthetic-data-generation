...
# Run     : python self_instruct_v8.py
#
# CHANGES vs v7:
...
OUT_JSON        = "/home/kritika/self_instruct/paraphrase_v8.json"
OUT_CSV         = "/home/kritika/self_instruct/paraphrase_v8.csv"
OUT_LOG         = "/home/kritika/self_instruct/paraphrase_v8_log.txt"
...
# FIX-22: Max retries per slot after a judge flag (hard-gate mode)
# If a slot fails judge verification this many times in a row, it counts
# as a style failure and the slot is abandoned (not added to output).
MAX_JUDGE_RETRIES   = 3
...
def make_generated_id(seed: dict, style_name: str, idx: int) -> str:
    ...
    return f"si_v8_{style_name[:6]}_{base}_{idx}"
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Self-Instruct Paraphrase v8 (English only, CSV input)")
    ...
    print(f"  Judge      : enable_thinking=False, max_tokens={JUDGE_MAX_TOKENS} (HARD GATE — flagged pairs retried up to {MAX_JUDGE_RETRIES}x)")
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        for style in STYLES:
            ...
            while style_done < TARGET_PER_STYLE:
                ...
                # ── FIX-21: Judge call (HARD GATE — flagged pairs are discarded
                #            and retried up to MAX_JUDGE_RETRIES times) ──────────
                ...
                # Inner retry loop: keep regenerating until judge passes
                # or we exhaust MAX_JUDGE_RETRIES attempts for this slot.
                while judge_retry_count <= MAX_JUDGE_RETRIES:
                    ...
                    if is_grounded:
                        # Judge passed — accept this generation
                        break

                    # Judge flagged — log, discard, and regenerate
                    judge_flag_count += 1
                    judge_retry_count += 1
                    log_lines.append(
                    ...
                    if judge_retry_count > MAX_JUDGE_RETRIES:
                        # Exhausted retries — treat as a style failure
                        print(f"⚠ FLAGGED ({judge_confidence}) — max judge retries exhausted, slot skipped")
                        break

                    # Regenerate a fresh candidate for this slot
                    ...
                    if result is None:
                        # Generation itself failed during judge retry — bail out of retry loop
                ...
                # After retry loop: only accept if judge passed
                if not is_grounded or result is None:
                    ...
                    continue   # back to while style_done < TARGET_PER_STYLE
                ...
                record = {
                    ...
                    # ── FIX-21: Judge fields (all saved pairs are grounded=True) ─
                    "judge_grounded"    : True,
                    "judge_confidence"  : judge_confidence,
                    "judge_issues"      : "",   # always clean — flagged were retried
                    # ─────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    '''Looking at the log carefully, I can see two distinct problems beyond the flagging issue we already fixed:

1. **`'NoneType' object has no attribute 'strip'`** — a crash in `call_model` when the API returns a response with no content, causing `resp.choices[0].message.content` to be `None`. Currently it hits an unhandled exception and wastes a retry attempt.

2. **High flag rate on specific styles** — `paraphrase_problem_description` (62%), `paraphrase_preventive` (53%), `paraphrase_cause` (43%) are the worst offenders. Looking at the flagged questions, the model keeps adding sowing timing, intercropping facts, and location-specific details not in the seed. The system prompt needs tighter per-style grounding constraints.

3. **`paraphrase_comparison` thrashing** — 26 calls for 4 pairs because the model keeps generating short answers (10–40w) that fail the 50w minimum. The seed often has limited comparable items, so the model gives up quickly.

Let me fix all three:**Fix 1 — NoneType crash: guard `message.content` before `.strip()`****Fix 2 — NoneType crash in `call_model`:****Fix 3 — Tighten style descriptions for the 3 high-flag styles + lower comparison floor:**Now fix the three high-flag style descriptions:**Fix 4 — Update all v8 → v9 references in file paths and IDs:**All fixes verified, syntax clean ✓. Let me copy to outputs:'''