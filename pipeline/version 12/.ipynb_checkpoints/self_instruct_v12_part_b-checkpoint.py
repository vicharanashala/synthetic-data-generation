# self_instruct_v12_part_b.py
# Purpose : Expand seed Q&A pairs into diverse paraphrase variants.
#           PART B — processes seeds 151 to 200 (indices 150–199).
#           Auto-resumes from last checkpoint if interrupted.
# Run     : python self_instruct_v12_part_b.py
#
# CHANGES vs v11:
...
OUT_JSON        = "/home/kritika/self_instruct/paraphrase_v12_part_b.json"
OUT_CSV         = "/home/kritika/self_instruct/paraphrase_v12_part_b.csv"
OUT_LOG         = "/home/kritika/self_instruct/paraphrase_v12_part_b_log.txt"

SEED_START          = 150  # inclusive index into the CSV (0-based)
SEED_END            = 200  # exclusive — processes seeds at index 150..199 (rows 151–200)
MAX_SEEDS           = 200  # kept for reference; actual slice is SEED_START:SEED_END
TARGET_PER_STYLE    = 5
...
def main():
    ...
    seeds = all_seeds[SEED_START:SEED_END]

    print(f"\n{'='*60}")
    print(f"  Self-Instruct Paraphrase v12 — PART B (seeds {SEED_START+1}–{SEED_END})")
    print(f"  Seeds      : {len(seeds)} (rows {SEED_START+1}–{SEED_END} of {len(all_seeds)} loaded)")
