...
OUT_JSONL       = "/home/kritika/self_instruct/conversational pipeline/workers/multiturn_from_conv.jsonl"
OUT_LOG         = "/home/kritika/self_instruct/conversational pipeline/workers/multiturn_from_conv_log.txt"
...
# SPEED CONFIG
MAX_CONCURRENT_WORKERS = 20   # Number of records to process simultaneously
...
1. STRICT LANGUAGE MATCHING (CRITICAL): You MUST match the exact language used in the "CONVERSATION SO FAR". 
   ...
   - IF THE CONVERSATION IS IN ENGLISH: Generate the follow-up question and answer in standard, practical English ONLY.
   - IF THE CONVERSATION IS IN HINGLISH: Generate the follow-up in natural, conversational Hinglish. 
     * DO NOT use formal/bookish Hindi.
     * DO use natural village farming Hinglish mixed with English terms (e.g., "spray karein", "dose kitni hai").
...
def call_model(seed_answer: str, conversation: list, turn_number: int, excluded: list):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if has_self_reference(ans) or has_self_reference(result["user_question"]): continue
            if not is_answer_length_ok(ans): continue
...
# ─── Threading & Checkpointing ────────────────────────────────────────────────
# Thread lock to prevent files from getting scrambled when multiple threads write
...
# ─── Record Processor Worker ──────────────────────────────────────────────────
def process_record(rec_idx, row, completed_keys, total_records):
    source_id = row.get("Answer ID") or row.get("source_id") or str(rec_idx)
    if source_id in completed_keys:
        return f"Skipped {source_id} (Already done)"
    ...
    if "Question Text" in row and "Answer Text" in row:
        ...
        return f"Skipped {source_id} (Unknown format)"
    ...
    for turn_num in range(1, N_FOLLOWUP_TURNS + 1):
        ...
        while attempts < MAX_RETRIES:
            ...
            if result and result.get("exhausted"): break
            if not result: continue

            followup_q, followup_a = result["user_question"], result["assistant_answer"]
            
            if is_duplicate(followup_q, seen_questions): continue
            ...
            if not verdict["grounded"]: continue
        ...
        if not success and not (result and result.get("exhausted")):
            break # Exit early if we completely fail to generate a valid turn
    ...
    return f"Completed [{rec_idx+1}/{total_records}] {crop} | {source_id} (Added {turns_added} turns)"
...
def main():
    ...
    print(f"Starting ThreadPoolExecutor with {MAX_CONCURRENT_WORKERS} parallel workers...\n")
    ...
    with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_WORKERS) as executor:
        ...
        for rec_idx, row in enumerate(records):
            futures.append(
                executor.submit(process_record, rec_idx, row, completed_keys, len(records))
            )
        
        # As threads finish, print their completion status
