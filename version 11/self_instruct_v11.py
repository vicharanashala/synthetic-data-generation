...
OUT_JSON        = "/home/kritika/self_instruct/paraphrase_v11.json"
OUT_CSV         = "/home/kritika/self_instruct/paraphrase_v11.csv"
OUT_LOG         = "/home/kritika/self_instruct/paraphrase_v11_log.txt"
...
def make_generated_id(seed: dict, style_name: str, idx: int) -> str:
    ...
    return f"si_v11_{style_name[:6]}_{base}_{idx}"   # FIX-36
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Self-Instruct Paraphrase v11 (English only, CSV input)")
    ...
    print(f"  FIX-33     : Dedup threshold 0.90 → 0.75 (word Jaccard).")
    print(f"  FIX-34     : paraphrase_comparison removed (36% grounding failure rate).")
    print(f"  FIX-35     : paraphrase_yes_no target capped at 3 (binary surface ceiling).")
    print(f"  FIX-36     : Output files paraphrase_v11.*, IDs si_v11_ prefix.")
    print(f"  (Inherited) Scope alignment, scope_overflow judge check, per-style word limits.")
    print(f"{'='*60}\n")
