...
N_WORKERS       = 1   # FIX-40: parallel workers
MODEL           = "Qwen/Qwen3-30B-A3B"
INPUT_FILE      = "/home/kritika/self_instruct/random_200_qa_1.csv"
OUT_JSON        = "/home/kritika/self_instruct/paraphrase_v13.json"
OUT_CSV         = "/home/kritika/self_instruct/paraphrase_v13.csv"
OUT_LOG         = "/home/kritika/self_instruct/paraphrase_v13_log.txt"
...
# ─── Model Client ─────────────────────────────────────────────────────────────
# client is now created inside each worker process — removed from global scope
# ─── Generation Caller ────────────────────────────────────────────────────────
...
def call_judge(seed_answer: str, generated_question: str,
               generated_answer: str, client=None) -> dict:
    ...
    return {"grounded": True, "scope_overflow": False,
            "confidence": "low", "issues": [], "judge_error": "max_retries"}
...
def run_worker(worker_id: int, seeds_chunk: list) -> None:
    """Process a subset of seeds. Each worker has its own client + checkpoint."""
    client_local = OpenAI(base_url=VLLM_URL, api_key="dummy")

    # Per-worker output files
    out_json = OUT_JSON.replace(".json", f"_worker{worker_id}.json")
    out_csv  = OUT_CSV.replace(".csv",  f"_worker{worker_id}.csv")
    out_log  = OUT_LOG.replace("_log.txt", f"_log_worker{worker_id}.txt")

    # Load this worker's checkpoint
    all_results, completed_source_ids = [], set()
    if os.path.exists(out_json):
        try:
            with open(out_json, "r", encoding="utf-8") as f:
                all_results = json.load(f)
            completed_source_ids = set(r["source_id"] for r in all_results)
            print(f"[Worker {worker_id}] Resumed — {len(all_results)} results, "
                  f"{len(completed_source_ids)} seeds done.")
        except Exception as e:
            print(f"[Worker {worker_id}] Checkpoint load failed: {e}. Starting fresh.")
    ...
    for seed_idx, seed in enumerate(seeds_chunk):
        if seed.get("id", "") in completed_source_ids:
            print(f"[Worker {worker_id}] Seed {seed_idx+1}/{len(seeds_chunk)} already done — skip")
            continue

        print(f"\n[Worker {worker_id}] Seed {seed_idx+1}/{len(seeds_chunk)}: "
              f"{seed.get('crop') or 'N/A'} | {seed.get('domain') or 'N/A'}")
        ...
        for style in STYLES:
            ...
            style_failures = 0
            call_count     = 0
            style_target   = TARGET_PER_STYLE_OVERRIDES.get(style["name"], TARGET_PER_STYLE)
            ...
            while style_done < style_target:
                ...
                result, detected_focus = call_model(
                    ...
                    consecutive_unknowns, client=client_local   # ← pass client directly
                )
                call_count += 1
                ...
                verdict = call_judge(
                    seed["answer"], result["generated_question"],
                    result["generated_answer"], client=client_local  # ← pass client directly
                )
                ...
                record = {
                    ...
                    "style_call_number"   : call_count,
                    "followup_focus"      : (detected_focus
                                            if style["name"] == "paraphrase_followup" else ""),
                    "answer_word_count"   : len(result["generated_answer"].split()),
                    "judge_grounded"      : True,
        ...
        save()
        print(f"[Worker {worker_id}] ✅ Seed {seed_idx+1} done — checkpoint saved")

    print(f"\n[Worker {worker_id}] FINISHED — {len(all_results)} pairs total.")
...
if __name__ == "__main__":
    ...
    # Split remaining 100 seeds (index 100-199) across 32 workers
    remaining_seeds = seeds[100:200]
    chunk_size = len(remaining_seeds) // N_WORKERS
    chunks = [remaining_seeds[i * chunk_size:(i + 1) * chunk_size] for i in range(N_WORKERS)]
    if len(remaining_seeds) % N_WORKERS:
        chunks[-1].extend(remaining_seeds[N_WORKERS * chunk_size:])

    processes = []
    for i in range(N_WORKERS):
        p = multiprocessing.Process(target=run_worker, args=(i, chunks[i]))
        p.start()
        processes.append(p)
    ...
    # Merge all worker outputs + existing first-100-seeds results
    print("\nMerging worker outputs...")
    merged = []
    # Load already completed first 100 seeds
    if os.path.exists(OUT_JSON):
        with open(OUT_JSON, "r", encoding="utf-8") as f:
            try:
                merged = json.load(f)
                print(f"  Loaded {len(merged)} existing pairs from first 100 seeds")
            except Exception:
                pass

    merged_logs = []
    for i in range(N_WORKERS):
        wj = OUT_JSON.replace(".json", f"_worker{i}.json")
        wl = OUT_LOG.replace("_log.txt", f"_log_worker{i}.txt")
        if os.path.exists(wj):
            with open(wj, "r", encoding="utf-8") as f:
                merged.extend(json.load(f))
        if os.path.exists(wl):
            with open(wl, "r", encoding="utf-8") as f:
                merged_logs.append(f.read())
    ...
    print(f"✅ Merged {len(merged)} total pairs → {OUT_JSON}")