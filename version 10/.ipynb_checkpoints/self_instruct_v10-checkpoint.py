...
OUT_JSON        = "/home/kritika/self_instruct/paraphrase_v10.json"
OUT_CSV         = "/home/kritika/self_instruct/paraphrase_v10.csv"
OUT_LOG         = "/home/kritika/self_instruct/paraphrase_v10_log.txt"
...
STYLES = [
...

    {
        ...
        "description": (
            ...
            "GROUNDING RULE: Do NOT add new facts not present in the seed answer. "
            "IMPORTANT: Treat compound product names (e.g. 'Carboxin 37.5% + "
            "Thiram 37.5%') as a SINGLE entity. Do NOT decompose compound names "
            "into their individual components and list them as separate chemicals."
        ),
    },
...
def make_generated_id(seed: dict, style_name: str, idx: int) -> str:
    ...
    return f"si_v10_{style_name[:6]}_{base}_{idx}"   # FIX-32
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Self-Instruct Paraphrase v10 (English only, CSV input)")
    ...
    print(f"  FIX-29     : Scope alignment — question→answer scope check in system")
    print(f"               prompt (Rule 11), yes_no + short style descriptions,")
    print(f"               and judge (CHECK 2 scope_overflow). Judge now receives")
    print(f"               generated_question in addition to seed + generated answer.")
    print(f"  FIX-30     : scope_overflow field saved in output JSON/CSV.")
    print(f"  FIX-31/32  : Output files paraphrase_v10.*, IDs si_v10_ prefix.")
    print(f"{'='*60}\n")

    all_results      = []
    log_lines        = []
    judge_flag_count = 0
    scope_overflow_count = 0  # FIX-30: track scope overflow rejections
    ...
    print(f"  Log         : {OUT_LOG}")
    print(f"  Note: All saved pairs passed both grounding AND scope alignment checks.")
